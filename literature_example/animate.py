...
example_mesh.load_from_info(len(tanke) + 1)
...
von_mis_stress_per_it = np.array(
    [
        cm.calculate_von_mises_stress(
            stress_per_it
        ) for stress_per_it in example_mesh.calculated_stress
    ]
)

# max_von_misses_stress = np.max(von_mis_stress_per_it)
max_von_misses_stress = 100
min_von_misses_stress = 0
...
def drawing_function(iterator):
    ...
    example_mesh.used_mesh.beam_height = \
        example_mesh.calculated_widths[iterator][0]

    example_mesh.used_mesh.beam_width_array[tanke] = \
        example_mesh.calculated_widths[iterator][1:]
    ...
    y_error = np.sum(
        (
            ...
            displacement[
                example_mesh.final_displacement_node_positions
            ][:, 1])**2,
        axis=0
    )

    io_ratio = example_mesh.used_mesh.final_displacement_array[:, 1:][0, 1] /\
        displacement[example_mesh.force_node_positions[0]][1]
